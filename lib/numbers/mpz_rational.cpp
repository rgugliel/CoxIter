...
MPZ_rational::MPZ_rational( mpz_class a, mpz_class b ) 
	: a( a ), b( b )
{
	update( );
}
...
MPZ_rational::MPZ_rational( string szRational )
{
	...
	iRegexpCount = reg.preg_match_all( "^[\\+]{0,1}([\\-]{0,1})([[:digit:]]+)$", szRational, results );
	if( iRegexpCount == -1 )
		throw( 0 );
	...
	iRegexpCount = reg.preg_match_all( "^[\\+]{0,1}([\\-]{0,1})([[:digit:]]+)\\/([[:digit:]]+)$", szRational, results );
	if( iRegexpCount == -1 )
		throw( 0 );
	else if( iRegexpCount > 0 )
	{
...
bool MPZ_rational::operator==( const int& i ) const
{
	return ( a == i && b == 1 );
}
