...
class CoxIter
{
	private:
		...
		vector< unsigned int > iPath; ///< chemin en cours (pour le DFS)
		 ...
		 * 	map< vector< vector< unsigned int > >, unsigned int><br>
		 * 		The key is "vector< vector< unsigned int > >": For each type of graph and each rank, how many times it occurs in the product<br />
		 ...
		 */
		vector< map<vector< vector< unsigned int > >, unsigned int> > graphsProductsCount_spherical;
	...
	public:
		...
		*/
		 ...
		 * 	\brief Retourne la somme alternée des composantes du f-vecteur
		 * 	\return Somme alternée des composantes du f-vecteur (int)
		 */
		 ...
		 * 	\brief Retourne un tableau de string contenant les entrées de 2*G
		 * 	\return Tableau
		 */
		vector< vector< string > > get_array_str_2_GramMatrix( ) const;
		
		/*!	\fn get_strGramMatrix
		 * 	\brief Retourne la matrice de Gram
		 * 	\return Matrice de Gram (string)
		 */
		 ...
		 * 	\brief Retourne le corps engendré par les entrées de la matrice de Gram
		 * 	\return Retourne le corps engendré par les entrées de la matrice de Gram (string)
		 */
		 ...
		 * 	\brief Retourne le label d'un sommet correspondant à son label
		 * 	\return Retourne le label d'un sommet correspondant à son label
		 */
		 ...
		 * 	\param iMat( const vector< vector< unsigned int > >& ) The matrix
		 */
	...
	private:
		...
		/*! \fn DFS
		 * \brief Cherche toutes les chaînes qui partent d'un sommet
		 * 
		 * Cette fonction appelle addGraph( ) à chaque fois qu'elle a une chaîne de longueur maximale
		 * 
		 * \param iRoot Sommet de départ
		 * \param iFrom Sommet précédent (si appel récursif depuis DFS) et iRoot sinon
		 */
		void DFS( unsigned int iRoot, unsigned int iFrom ); // fonction récursive (depuis DFS ou elle même)
		 ...
		 * 
		 * \param iPathTemp( vector< unsigned int >& ) Chemin actuel composant le An
		 * \param bVerticesLinkable( const vector< bool >& ) Ce qui est liable ou non au graphe
		 */
		void AnToEn_AnToTEn( const vector< unsigned int >& iPathTemp, const vector< bool >& bVerticesLinkable );
		
		/*!
		 * \fn AnToEn_AnToTEn( const vector< unsigned int >& iPathTemp, const vector< bool >& bVerticesLinkable, const bool& bSpherical, const unsigned int& iStart )
		 * \brief Essaie de construire des En depuis un An (applé depuis l'autre fonction)
		 * 
		 * \param iPathTemp ( const vector<unsigned int>& ) Chemin actuel composant le An
		 * \param bVerticesLinkable( const vector<bool> & ) Ce qui est liable ou non au graphe 	
		 * \param bSpherical( const bool& ) True si sphérique, false si euclidien
		 * \param iStart( const unsigned int& ) A partir de quel sommet on essaie de partir
		 */
		void AnToEn_AnToTEn( const vector< unsigned int >& iPathTemp, const vector< bool >& bVerticesLinkable, const bool& bSpherical, const unsigned int& iStart );
		
		/*!
		 * \fn B3ToF4_B4ToTF4
		 * \brief Essaie de construire un F4 depuis un B3
		 * 
		 * \param bVerticesBeginLinkable( const vector<bool> & ) Ce qui est liable ou non à une partie du B3
		 * \param iPathTemp (vector<unsigned int>) Chemin actuel composant le B3
		 * \param iVEnd Numéro du sommet de poids 4
		 */
		void B3ToF4_B4ToTF4( const vector<bool> &bVerticesBeginLinkable, vector<unsigned int> iPathTemp, const unsigned int &iVEnd );
		
		/*!	\fn computeGraphsProducts( GraphsListIterator grIt, vector< map<vector< vector< unsigned int > >, unsigned int> >* graphsProductsCount, const bool& bSpherical, GraphsProduct& gp, vector< bool >& bGPVerticesNonLinkable )
		 * 	\brief Cherche à créer les produits à partir des listes de graphes
		 * 
		 * 	Cette méthode est privée (et est appelée depuis l'autre findGraphProducts) ou de manière récursive
		 * 
		 * 	\param grIt( GraphsListIterator ): Itérateur sur la liste de graphes
		 * 	\param graphsProductsCount( vector< map<vector< vector< unsigned int > >, unsigned int> >* ) Pointeur vers la liste des graphes à considérer (i.e. graphsProductsCount_spherical ou graphsProductsCount_euclidean)
		...
		/*!	\fn growthSeries_symbolExponentFromProduct( const vector< vector< unsigned int > >& iProduct, vector< unsigned int >& iSymbol, unsigned int& iExponent ) const
