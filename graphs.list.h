...
public:
   ...
   */
  GraphsList(size_t iVerticesCount,
   ...
   *
   * 	\param iVertices(const vector< unsigned int > &): tableau contenant les
   ...
   * bSpherical(bool): true si graphe sphérique, false sinon \param
   ...
   * le G_2, le poids)
   */
  void addGraph(const vector<short unsigned int> &iVertices,
   ...
   * 	\param iGraphIndex(size_t &): Index du graphe actuel (i.e. position dans
