...
bool Tests::readGraphsFile(string strInput)
{
	...
	while(getline(fileIn, szLine)) // tant que....
	{
		...
		regexpRes.clear();
		if (regexp.preg_match_all("\"([^\"]+)\"", szLine, regexpRes) == 0) // If we cannot read the file
		{
		...
		if (regexp.preg_match_all("([-]{0,1})([[:digit:]/]+)", szLineWork, regexpRes) != 0)
		{
			...
			if (regexp.preg_match_all("([[:digit:]]+)", szNumber, regexpRes) != 0) // If there is at least one digit
			{
				try{
...
unsigned int Tests::get_iTestsCount() const
{
	return tests.size();	
}

string Tests::get_strError() const
{
	return strError;
}
...
void Tests::bRunTests_init()
{
	...
	strTestDescription["growthSeriesDenomDimOdd"] = "Denom. growth series vanish at 1";
...
bool Tests::bRunTests_computations(const unsigned int& iTestIndex, CoxIter* ci)
{
	ci->exploreGraph();
...
void Tests::bRunTests_growth(const unsigned int& iTestIndex, CoxIter* ci)
{
	...
	if (tests[iTestIndex].strGrowthRate != "")
	{
		...
		else
		{
			...
			bRunTests_error(iTestIndex, "growth rate", tests[iTestIndex].strGrowthRate, strGrowthRate);
		}
	}
	...
	if (ci->get_iIsFiniteCovolume()> 0 )
	{
		...
		{
			if (tests[iTestIndex].growthSeries_iCyclotomicNumerator == iCyclotomic && tests[iTestIndex].growthSeries_iPolynomialDenominator == iDenom)
			{
		...
		if (ci->get_iDimension() % 2) // n is odd, the denominator should vanish in 1
		{
			...
			else
			{
				...
				of << "Error\t " << tests[iTestIndex].szFile << endl;
				of << "\t\tGrowth series test: the denominator should vanish at 1" << endl;
			}
		}
		else // n even, we should find the euler characteristics
		{
			...
			if (MPZ_rational(iTotalDenom, iTotalNum) == ci->get_brEulerCaracteristic())
			{
			...
			else
			{
				...
				of << "Error\t " << tests[iTestIndex].szFile << endl;
				of << "\t\tGrowth series test: different from Euler characteristic" << endl;
			}
		}
	}
}

void Tests::bRunTests_signature(const unsigned int& iTestIndex, CoxIter* ci, const unsigned int& iDim)
{
	...
	if (iSignatureComputed == 1 && ci->get_iDimension())
	{
		if (iSignature[0] != ci->get_iDimension() || iSignature[1] != 1)
		{
			...
			of << "Error\t " << tests[iTestIndex].szFile << endl;
			of << "\t\tSignature computed: (" << iSignature[0] << "," << iSignature[1] << "," << iSignature[2] << ")" << endl;
		}
		else
		{
	...
	else if (iSignatureComputed == 0 && tests[iTestIndex].szFile.find("roberts", 0) != string::npos)
	{
		iTestsSucceded["signature"][1]++;
		of << "Error\t " << tests[iTestIndex].szFile << endl;
...
void Tests::bRunTests_arithmeticity(const unsigned int& iTestIndex, CoxIter* ci)
{
	int iArithmeticity;
	...
	if ((iArithmeticity == 1 && tests[iTestIndex].bIsArithmetic) || (iArithmeticity == 0 && !tests[iTestIndex].bIsArithmetic))
	{
	...
	else
	{
		...
		bRunTests_error(iTestIndex, "arithmeticity", tests[iTestIndex].bIsArithmetic ? "yes" : "no", strIntToString(iArithmeticity));
	}
}

void Tests::bRunTests_cocompactness_cofiniteness(const unsigned int& iTestIndex, CoxIter* ci)
{
	...
	if (!((iFiniteVolume == 1 && tests[iTestIndex].bIsFiniteVolume) || (iFiniteVolume == 0 && !tests[iTestIndex].bIsFiniteVolume)))
	{
		iTestsSucceded["cofiniteness"][1]++;
		bRunTests_error(iTestIndex, "cofiniteness test", tests[iTestIndex].bIsFiniteVolume ? "yes" : "no", strIntToString(iFiniteVolume));
	}
	else
	{
	...
	if (tests[iTestIndex].bTestCompacity)
	{
		...
		if ((iCompacity == 1 && tests[iTestIndex].bIsCompact) || (iCompacity == 0 && !tests[iTestIndex].bIsCompact))
		{
		...
		else
		{
			...
			bRunTests_error(iTestIndex, "cocompactness test", tests[iTestIndex].bIsCompact ? "yes" : "no", strIntToString(iCompacity));
		}
	}
}
...
void Tests::bRunTests_Euler(const unsigned int& iTestIndex, CoxIter* ci)
{
	if (tests[iTestIndex].bTestEuler)
	{
		if (ci->get_brEulerCaracteristic() != tests[iTestIndex].brResult)
		{
			...
			bRunTests_error(iTestIndex, "Euler characteristic", tests[iTestIndex].brResult.to_string(), ci->get_brEulerCaracteristic().to_string());
		}
		else
		{
	...
	else if (ci->get_iDimension() % 2 && tests[iTestIndex].bIsFiniteVolume)
	{
		...
		{
			iTestsSucceded["euler"][1]++;
			bRunTests_error(iTestIndex, "Euler characteristic", "0", ci->get_brEulerCaracteristic().to_string());
		}
		else
		{
			iTestsSucceded["euler"][0]++;
...
void Tests::bRunTests_FVector(const unsigned int& iTestIndex, CoxIter* ci)
{
	if (ci->get_iIsFiniteCovolume() > 0)
	{
		...
		else
		{
			...
			bRunTests_error(iTestIndex, "alt. sum of components of f-vector", to_string(ci->get_iDimension() % 2 ? 2 : 0), to_string(ci->get_iFVectorAlternateSum()));
		}
	}
	...
	if (tests[iTestIndex].bTestFVector)
	{
		...
		{
			iTestsSucceded["fv"][1]++;
			bRunTests_error(iTestIndex, "f-vector", implode(",", tests[iTestIndex].iFVector), implode(",", ci->get_iFVector()));
		}
	}
}

void Tests::bRunTests_error(const unsigned int& iTestIndex, const string& strTest, const string& strExpected, const string& strComputed)
{
...
void Tests::bRunTests_displayInfo()
{
	...
	cout << "------------------------------------------------------" << endl;
	cout << setw(32) << left << "Test" << " | #success | #error |" << endl;
	cout << "------------------------------------------------------" << endl;
	
	for (map<string, string>::const_iterator it(strTestDescription.begin()); it != strTestDescription.end(); it++)
	{
