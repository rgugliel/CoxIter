...
class Tests
{
	private:
		...
		map<string, array<unsigned int, 2>> iTestsSucceded; ///< For each test, number of success, failure
	...
	private:
		...
		void bRunTests_cocompactness_cofiniteness(const unsigned int& iTestIndex, CoxIter* ci);
		void bRunTests_growth(const unsigned int& iTestIndex, CoxIter* ci);
		void bRunTests_signature(const unsigned int& iTestIndex, CoxIter* ci, const unsigned int& iDim);
		...
		void bRunTests_error(const unsigned int& iTestIndex, const string& strTest, const string& strExpected, const string& strComputed);
