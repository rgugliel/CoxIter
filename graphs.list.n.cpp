...
void GraphsListN::addGraph(vector<short unsigned int> iVertices,
                           const vector<bool> &bVerticesLinkable,
                           const unsigned int &iType, bool bSpherical,
  ...
  if (bSpherical) {
    ...
    if (iType == 0 && iDataSupp) {
      ...
      int iDirection(
          iVertices[!iMinIndex ? (iVerticesCount - 1) : (iMinIndex - 1)] >
                  iVertices[iMinIndex == (int)(iVerticesCount - 1)
                                ? 0
                                : (iMinIndex + 1)]
              ? 1
              : -1);
...
// le vecteur iVerticesBase contient la base (i.e. sans les 4 extrémités)
...
#else
        vector<short unsigned int> iVerticesBase(
            iVertices.begin() + 1, iVertices.begin() + iVerticesCount -
        ...
        if (iVerticesBase[0] > iVerticesBase[iVerticesCount - 5]) {
          ...
          iVerticesBase.insert(
              iVerticesBase.begin(),
              max(iVertices[iVerticesCount - 3], iVertexSupp2));
          iVerticesBase.insert(
              iVerticesBase.begin(),
              min(iVertices[iVerticesCount - 3], iVertexSupp2));
        } else {
          // ajout des 4 extrémités
          iVerticesBase.push_back(
              min(iVertices[iVerticesCount - 3], iVertexSupp2));
          iVerticesBase.push_back(
              max(iVertices[iVerticesCount - 3], iVertexSupp2));
          iVerticesBase.insert(iVerticesBase.begin(),
                               max(iVertices[0], iVertexSupp1));
          iVerticesBase.insert(iVerticesBase.begin(),
                               min(iVertices[0], iVertexSupp1));
        }
    ...
    } else if (iType == 4) // \tilde En
    {
      ...
      {
        if (iVertices.front() >
            iVertices.back()) // la base du \tilde E7 est symétrique
          reverse(iVertices.begin(), iVertices.end());
  ...
  Graph g(iVertices, ptr_map_vertices_indexToLabel, bVerticesLinkable, iType,
          bSpherical, iDataSupp);
...
unsigned int GraphsListN::get_iVerticesCount() const { return iVerticesCount; }
